...
-
ql.CompositeQuote(
*element1: ql.QuoteHandle*,*element2: ql.QuoteHandle*,*f*)
...
class is a key component of the C++ QuantLib library. Conceptually, a Handle behaves like a **double pointer** — it essentially wraps a `shared_ptr`

(for more details, see the Handling dependencies section).
...
In Python, the naming convention for these handle classes follows the pattern `<ClassName>Handle`

— for example, `YieldTermStructureHandle`
...
Handles are especially useful when the underlying object may change dynamically. For instance, if the base term structure changes and we need to reprice a bond, we can use a `RelinkableHandle`

to easily update the reference. By calling the `linkTo`

method, the handle can be relinked to a new term structure, after which we can simply request the bond’s `NPV()`
